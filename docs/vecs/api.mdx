...
If you don't have a Postgres database with the pgvector ready, see [hosting](/vecs/hosting) for easy options.
...
Before you can interact with vecs, create the client to communicate with Postgres. If you haven't started a Postgres instance yet, see [hosting](/hosting).
```python
import vecs
...
For a complete reference, see the [metadata guide](/vecs/concepts/metadata).
...
For a complete list of available adapters, see [built-in adapters](/vecs/concepts/adapters#built-in-adapters).
