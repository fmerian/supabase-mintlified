...
Indexes are tools for optimizing query performance of a [collection](/concepts/collections).

Collections can be [queried](/api#query) without an index, but that will emit a python warning and should never be done in production.
