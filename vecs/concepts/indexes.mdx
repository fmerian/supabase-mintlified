...
Indexes are tools for optimizing query performance of a [collection](/vecs/concepts/collections).

Collections can be [queried](/vecs/api#query) without an index, but that will emit a python warning and should never be done in production.
...
<Info>
  ...
  For a few thousand records expect sub-minute a response in under a minute. It may take a few minutes for larger collections.
</Info>
